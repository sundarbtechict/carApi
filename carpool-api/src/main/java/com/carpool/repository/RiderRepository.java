...
public interface RiderRepository extends CrudRepository<Rider , Integer> ,JpaRepository<Rider, Integer>
{
	...
	@Modifying
	@Query(value="INSERT INTO `riderschedule`(`scheduleID`, `duration`, `from`, `to`, `distance`)"
			+ "VALUES (:id, SEC_TO_TIME(:duration), :from, :to, :distance)", nativeQuery = true)
	@Transactional
	public void insertRiderSchedule(@Param("id") int id, @Param("duration") long duration, @Param("from") String from,
			@Param("to") String to, @Param("distance") double distance);
	
	@Modifying
	@Query(value="INSERT INTO `driverschedule`(`driverscheduleID`, `duration`, `from`, `to`, `distance`, `seats_left`, `seats_total`)"
			+ "VALUES (:id, SEC_TO_TIME(:duration), :from, :to, :distance, :seats, :seats)", nativeQuery = true)
	@Transactional
	public void insertDriverSchedule(@Param("id") int id, @Param("duration") long duration, @Param("from") String from,
			@Param("to") String to, @Param("distance") double distance, @Param("seats") int seats);
	...
	@Query(value="SELECT driverschedule.driverscheduleID, riderschedule.scheduleID,driverschedule.from ,riderschedule.from as waypoints, "
			...
			+ "INNER JOIN riderschedule ON riderschedule.scheduleID=:id AND"
			+ " TIMEDIFF(riderschedule.dateTime,driverschedule.dateTime) <= MAKETIME(0,5,0) "
